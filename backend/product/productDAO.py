...
class productDAO:
    ...
    def product_reg(p_id, p_name, brand_id, category_id, price, image, location):
        con, cur = None, None
        try:
            con, cur = kimDBmanager.makeConCur("KIMCR/1@195.168.9.126:1521/xe")
            cur.execute("""
                INSERT INTO product_M 
                (p_id, p_name, brand_id, category_id, price, image_url, store_location)
                VALUES (:1, :2, :3, :4, :5, :6, :7)
            """, (p_id, p_name, brand_id, category_id, price, image, location))
            con.commit()
            return {"result": "상품 등록 완료"}
        except Exception as e:
            print("조회 실패:", e)
            return {"result": "조회 실패"}
        finally:
